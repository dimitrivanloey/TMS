...
def sticks_missing(request):
    ...
    arkle_entries = Arkle_Entry.objects.order_by('arkle').filter(date_added__gte=dt.now(tz=timezone.utc)-timedelta(days=30))
    denman_entries = Denman_Entry.objects.order_by('denman').filter(date_added__gte=dt.now(tz=timezone.utc)-timedelta(days=30))
    enable_entries = Enable_Entry.objects.order_by('enable').filter(date_added__gte=dt.now(tz=timezone.utc)-timedelta(days=30))
    winx_entries = Entry.objects.order_by('winx').filter(date_added__gte=dt.now(tz=timezone.utc)-timedelta(days=30))
    frankel_entries = Frankel_Entry.objects.order_by('frankel').filter(date_added__gte=dt.now(tz=timezone.utc)-timedelta(days=30))
    kauto_entries = Kauto_Entry.objects.order_by('kauto').filter(date_added__gte=dt.now(tz=timezone.utc)-timedelta(days=30))
    other_entries = Other_Entry.objects.order_by('other').filter(date_added__gte=dt.now(tz=timezone.utc)-timedelta(days=30))
